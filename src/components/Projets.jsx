...
const Projets = () => {
      ...
      <div className="space-y-16">
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-12">
          <div className="space-y-4">
            ...
            <p className="text-sm leading-relaxed">Vision globale du processus d'analyse, depuis la compréhension des besoins jusqu'à la production d'un outil automatisé.</p>
          </div>
          <div className="space-y-4">
            <h2 className="text-xl font-bold uppercase border-b-4 border-black pb-2">Impact sur mon parcours</h2>
            <p className="text-sm leading-relaxed">Choix du parcours <span className="font-bold">VCOD</span> suite à la découverte du pilotage via outils décisionnels.</p>
          </div>
        </section>
      </div>
    </motion.div>
      ...
      <div className="space-y-16">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="lg:col-span-5 space-y-8 pt-10 text-black">
            ...
            <p className="text-sm leading-relaxed">Migration vers <span className="font-bold">PostgreSQL</span> pour permettre un accès concurrentiel sécurisé.</p>
            <h3 className="text-xl font-bold uppercase flex items-center gap-2"><ShieldCheck size={20}/> Sécurisation</h3>
            <p className="text-sm leading-relaxed">Mise en place de politiques de <span className="font-bold">Row Level Security</span>.</p>
          </div>
        </section>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-12">
          <div className="space-y-4">
            <h2 className="text-xl font-bold uppercase border-b-4 border-black pb-2">Apports personnels</h2>
            <p className="text-sm leading-relaxed">Gestion d'une application web full-stack (front-end et back-end) et sécurisation cloud.</p>
          </div>
          <div className="space-y-4">
            <h2 className="text-xl font-bold uppercase border-b-4 border-black pb-2">Impact sur mon parcours</h2>
            <p className="text-sm leading-relaxed">Renforcement de mon intérêt pour l'ingénierie des données et la cybersécurité.</p>
          </div>
        </section>
      </div>
    </motion.div>
  ...
  const SAE502DetailView = () => (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-6xl mx-auto p-8 md:p-16 text-black bg-white">
      ...
      <div className="space-y-16">
        <section className="space-y-6">
          <p className="text-lg leading-relaxed font-medium">Migration d'une base relationnelle vers MongoDB pour optimiser les performances analytiques.</p>
          <ul className="space-y-3 border-l-4 border-black pl-6 font-bold">
            <li>• Compétence : Processus ETL (Python/Pandas)</li>
            <li>• Compétence : Modélisation documentaire</li>
            <li>• Compétence : Agrégation MongoDB</li>
          </ul>
        </section>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="lg:col-span-5 space-y-8 pt-10 text-black">
            ...
            <p className="text-sm leading-relaxed">Utilisation de structures imbriquées et de snapshots clients.</p>
          </div>
        </section>
      </div>
    </motion.div>
        ...
        <section className="text-center pb-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 text-left">
            {projetsPersonnels.map((p) => (
              <div key={p.id} onClick={() => setView(p.id)} className="border-2 border-black p-10 bg-white hover:bg-black group transition-all flex flex-col justify-between shadow-sm hover:shadow-xl cursor-pointer">
                <div>
                  <h3 className="text-2xl font-bold mb-4 uppercase text-black group-hover:text-white">{p.title}</h3>
                  <p className="text-sm mb-8 leading-relaxed font-bold text-black group-hover:text-white">{p.desc}</p>
                </div>
                <div className="flex flex-wrap gap-2">
                  {p.tools.map(t => <span key={t} className="px-4 py-1.5 bg-gray-100 border border-black text-black text-[10px] font-black uppercase group-hover:bg-white/20 group-hover:text-white">{t}</span>)}
                </div>
              </div>
            ))}
          </div>
